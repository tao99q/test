...
//2.将JSON字符串转为JSON对象
    ...
    function setCss(curEle, attr, val) {
        if (attr == "opacity") {
            ...
            curEle.style.filter = "alpha(opactiy=" + (val * 100) + ")";
            return;
        }
        if (attr == "float") {
        ...
        var reg = /^width|height|top|left|bottom|right|(padding|margin)(Top|Left|Bottom|Right)?$/;
        if (reg.test(attr)) {
