...
Drag.prototype.range = function (objRange) {
    ...
    //让这个范围在move中起作用，在这里无法修改move方法，只能向move方法执行的时候也给他添加另一个方法
    //增加一个自定义事件，让这个事件在move中执行
...
Drag.prototype.addRange = function (e) {
    ...
    var l = e.clientX - this.x;
    var t = e.clientY - this.y;
    l = l < this.objRange.minL ? this.objRange.minL : (l > this.objRange.maxL ? this.objRange.maxL : l);
    t = t < this.objRange.minT ? this.objRange.minT : (t > this.objRange.maxT ? this.objRange.maxT : t);
    this.ele.style.left = l + "px";
    this.ele.style.top = t + "px";
}
